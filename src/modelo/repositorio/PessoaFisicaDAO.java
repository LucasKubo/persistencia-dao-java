...
public class PessoaFisicaDAO extends PessoaDAO {
	public int criarPessoaFisica(PessoaFisica p) throws SQLException{
		...
		try {
			...
			if(id != 0) {
				...
				PreparedStatement ps = super.conn.prepareStatement("INSERT INTO pessoafisica (id,cpf,nascto)" + "VALUES(?,?,?)");
	...
	public PessoaFisica recuperarPessoaFisicaPorId(int id) {
		...
		try {
			Pessoa p = super.recuperarPessoaPorId(id);
			pf = new PessoaFisica(p.getNomePessoa(), p.getEnderecoPessoa(), p.getCepPessoa(), p.getTelefonePessoa(), p.getRendaPessoa(), 0l, null);
			int idPessoaFisica = super.recuperarPessoaPorId(id).getIdPessoa();
			
			PreparedStatement ps = super.conn.prepareStatement("SELECT * FROM pessoafisica WHERE (id = ?)");
